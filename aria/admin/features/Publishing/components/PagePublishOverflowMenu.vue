...
import type { PagePublishStatus } from "./PagePublishSplitButton.vue";
...
const props = withDefaults(
  defineProps<{
    ...
    duplicateForbiddenMessage?: string;
...
const schedulePopoverConfirmLabel = computed(() =>
  schedulePopoverMode.value === "reschedule"
    ? t("pages.detail.update")
    : t("pages.detail.schedule"),
);
...
<template>
  ...
  >
    <template #anchor>
      <HeaderActionDropdownTooltip :label="t('pages.detail.moreActions')">
        <DropdownMenu v-model:open="isDropdownOpen">
          <DropdownMenuTrigger as-child>
            <Button
              ...
              :aria-label="t('pages.detail.publishingOptions')"
            >
              <span
          ...
          >
            <PageMenuSectionLabel first>{{ t("pages.menu.status") }}</PageMenuSectionLabel>
            ...
            >
              ...
              {{ isBusy ? t("cms.unpublishing") : t("pages.action.unpublish") }}
            </DropdownMenuItem>
            ...
            >
              ...
              />
              {{ isArchived ? t("pages.action.unarchive") : t("pages.action.archive") }}
            </DropdownMenuItem>

            <PageMenuSectionLabel>{{ t("pages.menu.page") }}</PageMenuSectionLabel>

            <DropdownMenuItem
              :disabled="isBusy || !canDuplicate"
              :title="duplicateForbiddenMessage || undefined"
              @click="emit('duplicate')"
            >
              <span
                :class="[studioIcons.duplicate, 'mr-2 size-3.5 shrink-0']"
              />
              {{ t("pages.action.duplicate") }}
            </DropdownMenuItem>

            <template v-if="canDelete">
              <PageMenuSectionLabel>{{ t("pages.menu.remove") }}</PageMenuSectionLabel>
