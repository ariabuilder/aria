...
export function useGoogleModels() {
  async function refresh(force = false): Promise<CatalogModel[]> {
    ...
    error.value = null;
    try {
      const { data, error: actionError } = await actions.agent.listGoogleModels(
        {},
      );
      ...
      error.value =
        err instanceof Error ? err.message : "Failed to load Google AI models";
      return [];
    } finally {
