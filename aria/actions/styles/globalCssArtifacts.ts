import { camelToKebab } from "../../lib/types/classes";
import {
  buildGoogleFontsURL,
...
function buildGlobalStylesCss(designSystem: UniversalDesignSystem): string {
...
export async function buildGeneratedDocumentCss(
  ...
  return [motionCss, responsiveCss]
    .filter(Boolean)
    .join("\n\n")
    .trim();
}
...
function extractCoreBaseCss(baseCss: string): string {
  const marker = ARIA_DOCUMENT_RESET_CSS.trim();
  const markerIndex = baseCss.indexOf(marker);

  if (markerIndex === -1) {
    return baseCss.trim();
  }

  return baseCss.slice(0, markerIndex).trim();
}
...
export function buildStageRenderStylesData(input: {
  ...
  const coreBaseCss = extractCoreBaseCss(stored.baseCSS || "");
...
export function buildStoredRenderStylesData(
  ...
  const globalCSS =
    ...
    assembleGlobalCss({
      coreBaseCss: extractCoreBaseCss(baseCSS),
      utilityCSS,
      customClassesCSS,
      contextRulesCSS,
    });
...
function finalizeGlobalCSSArtifactsSnapshot(input: {
  ...
  const coreBaseCss = extractCoreBaseCss(rawBaseCss);
  const trimmedDocumentCss = generatedDocumentCss.trim();
  const resolvedCoreBaseCss =
    coreBaseCss.length > 0 ? coreBaseCss : rawBaseCss.trim();
  const resolvedDocumentCss =
    trimmedDocumentCss.length > 0
      ? trimmedDocumentCss
      : rawBaseCss.slice(resolvedCoreBaseCss.length).trim();
