...
import { BUTTON_VARIANT_ATTRIBUTE as BUTTON_VARIANT_DATA_ATTRIBUTE } from "../../blocks/buttonVariants";
import { normalizeContainerNodeType } from "../../blocks/containerTypes";
...
import { parseNavigationProps } from "../../blocks/navigationSchema";
import { projectManagedImage } from "./managedImage";
import {
...
export function compileCanonicalAttributes(props: JsonObject): string {
  const attrs: string[] = [];
...
export function compileCanonicalContent(props: JsonObject): string {
  const structuredContent = renderStructuredTextContent(props.content);
  if (structuredContent !== null) return structuredContent;
  if (props.content) return escapeHTML(String(props.content));
  if (props.text) return escapeHTML(String(props.text));
  if (props.label) return escapeHTML(String(props.label));
  return "";
}
  ...
  const childLines = node.children
    .map((child, index) => {
      if (index !== linkedTextChildIndex) {
        return compileCanonicalNodeHtml(
          child,
          indent + 1,
          breakpoints,
          "inline",
      ...
      const linkedChildHtml = compileCanonicalNodeHtml(
        stripTextLinkWrapperPropsFromNode(child),
        indent + 2,
        breakpoints,
        "inline",
...
export function compileCanonicalTag(
  node: BuilderNode,
  props: JsonObject,
...
export function compileCanonicalNodeHtml(
  node: BuilderNode,
  indent: number = 0,
  breakpoints: BreakpointDefinition[] = DEFAULT_BREAKPOINTS,
  ...
  const tag = compileCanonicalTag(node, renderProps, renderContext);
  ...
  const propsStr = compileCanonicalAttributes(attributeProps);
  if (propsStr) {
    attrs.push(propsStr);
  }
  ...
  const content = compileCanonicalContent(renderProps);
